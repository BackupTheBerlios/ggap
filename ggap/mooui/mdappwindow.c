...
static const char  *setting                         (MdAppWindow    *window,
                                                     const char     *s);
static void         init_prefs                      (MdAppWindow    *window);
static GtkToolbarStyle get_toolbar_style            (MdAppWindow    *window);


static const char  *md_app_window_class_get_id      (MdAppWindowClass *klass);
static const char  *md_app_window_class_get_name    (MdAppWindowClass *klass);

static GObject     *md_app_window_constructor       (GType           type,
                                                     guint           n_props,
                                                     GObjectConstructParam *props);
static void         md_app_window_dispose           (GObject        *object);
static void         md_app_window_set_property      (GObject        *object,
                                                     guint           prop_id,
                                                     const GValue   *value,
                                                     GParamSpec     *pspec);
static void         md_app_window_get_property      (GObject        *object,
                                                     guint           prop_id,
                                                     GValue         *value,
                                                     GParamSpec     *pspec);

static void         md_app_window_set_id            (MdAppWindow    *window,
                                                     const char     *id);
static void         md_app_window_create_class_actions
                                                    (MdAppWindow    *window);
static void         md_app_window_add_action        (MdAppWindow    *window,
                                                     const char     *group,
                                                     GtkAction      *action);
static void         md_app_window_remove_action     (MdAppWindow    *window,
                                                     const char     *action_id);

static gboolean     md_app_window_key_press_event   (GtkWidget      *widget,
                                                     GdkEventKey    *event);
static gboolean     md_app_window_delete_event      (GtkWidget      *widget,
                                                     GdkEventAny    *event);

static gboolean     md_app_window_save_size         (MdAppWindow    *window);

static void         md_app_window_update_ui         (MdAppWindow    *window);
static void         md_app_window_update_menubar    (MdAppWindow    *window);
static void         md_app_window_update_toolbar    (MdAppWindow    *window);

static void         md_app_window_shortcuts_prefs_dialog(MdAppWindow    *window);

static void         md_app_window_set_menubar_visible   (MdAppWindow    *window,
                                                         gboolean        visible);
static void         md_app_window_set_toolbar_visible   (MdAppWindow    *window,
                                                         gboolean        visible);

static GtkAction   *create_toolbar_style_action     (MdAppWindow    *window,
                                                     gpointer        dummy);

static void         md_app_window_set_focus         (GtkWindow      *window,
                                                     GtkWidget      *widget);
static void         md_app_window_disconnect_eo_widget
                                                    (MdAppWindow    *window);
static void         md_app_window_action_cut        (MdAppWindow    *window);
static void         md_app_window_action_copy       (MdAppWindow    *window);
static void         md_app_window_action_paste      (MdAppWindow    *window);
static void         md_app_window_action_delete     (MdAppWindow    *window);
static void         md_app_window_action_select_all (MdAppWindow    *window);
...
/* MD_TYPE_APP_WINDOW */
G_DEFINE_TYPE (MdAppWindow, md_app_window, GTK_TYPE_WINDOW)
static gpointer md_app_window_grand_parent_class;


static void
md_app_window_class_init (MdAppWindowClass *klass)
{
    ...
    md_app_window_grand_parent_class = g_type_class_peek_parent (md_app_window_parent_class);

    gobject_class->constructor = md_app_window_constructor;
    gobject_class->dispose = md_app_window_dispose;
    gobject_class->set_property = md_app_window_set_property;
    gobject_class->get_property = md_app_window_get_property;

    widget_class->delete_event = md_app_window_delete_event;
    widget_class->key_press_event = md_app_window_key_press_event;

    window_class->set_focus = md_app_window_set_focus;

    md_app_window_class_set_id (klass, "MdAppWindow", "Window");

    md_app_window_class_new_action (klass, "ConfigureShortcuts", NULL,
                                 "label", _("Configure _Shortcuts..."),
                                 "no-accel", TRUE,
                                 "stock-id", MOO_STOCK_KEYBOARD,
                                 "closure-callback", md_app_window_shortcuts_prefs_dialog,
                                 NULL);

    md_app_window_class_new_action (klass, "ShowToolbar", NULL,
    ...
    md_app_window_class_new_action (klass, "ShowMenubar", NULL,
                                 "action-type::", MOO_TYPE_TOGGLE_ACTION,
    ...
    md_app_window_class_new_action (klass, "Cut", NULL,
                                 "display-name", GTK_STOCK_CUT,
                                 "label", GTK_STOCK_CUT,
                                 "tooltip", GTK_STOCK_CUT,
                                 "stock-id", GTK_STOCK_CUT,
                                 "accel", "<ctrl>X",
                                 "closure-callback", md_app_window_action_cut,
                                 "condition::sensitive", "meo-can-cut",
                                 NULL);

    md_app_window_class_new_action (klass, "Copy", NULL,
                                 "display-name", GTK_STOCK_COPY,
                                 "label", GTK_STOCK_COPY,
                                 "tooltip", GTK_STOCK_COPY,
                                 "stock-id", GTK_STOCK_COPY,
                                 "accel", "<ctrl>C",
                                 "closure-callback", md_app_window_action_copy,
                                 "condition::sensitive", "meo-can-copy",
                                 NULL);

    md_app_window_class_new_action (klass, "Paste", NULL,
                                 "display-name", GTK_STOCK_PASTE,
                                 "label", GTK_STOCK_PASTE,
                                 "tooltip", GTK_STOCK_PASTE,
                                 "stock-id", GTK_STOCK_PASTE,
                                 "accel", "<ctrl>V",
                                 "closure-callback", md_app_window_action_paste,
                                 "condition::sensitive", "meo-can-paste",
                                 NULL);

    md_app_window_class_new_action (klass, "Delete", NULL,
                                 "display-name", GTK_STOCK_DELETE,
                                 "label", GTK_STOCK_DELETE,
                                 "tooltip", GTK_STOCK_DELETE,
                                 "stock-id", GTK_STOCK_DELETE,
                                 "closure-callback", md_app_window_action_delete,
                                 "condition::sensitive", "meo-can-delete",
                                 NULL);

    md_app_window_class_new_action (klass, "SelectAll", NULL,
                                 "display-name", GTK_STOCK_SELECT_ALL,
                                 "label", GTK_STOCK_SELECT_ALL,
                                 "tooltip", GTK_STOCK_SELECT_ALL,
                                 "stock-id", GTK_STOCK_SELECT_ALL,
                                 "accel", "<ctrl>A",
                                 "closure-callback", md_app_window_action_select_all,
                                 "condition::sensitive", "meo-can-select-all",
                                 NULL);

    md_app_window_class_new_action_custom (klass, TOOLBAR_STYLE_ACTION_ID, NULL,
    ...
    g_object_class_install_property (gobject_class, PROP_MEO_CAN_COPY,
        g_param_spec_boolean ("meo-can-copy",
                              "meo-can-copy",
                              "meo-can-copy",
                              FALSE,
                              G_PARAM_READABLE));

    g_object_class_install_property (gobject_class, PROP_MEO_CAN_CUT,
        g_param_spec_boolean ("meo-can-cut",
                              "meo-can-cut",
                              "meo-can-cut",
                              FALSE,
                              G_PARAM_READABLE));

    g_object_class_install_property (gobject_class, PROP_MEO_CAN_PASTE,
        g_param_spec_boolean ("meo-can-paste",
                              "meo-can-paste",
                              "meo-can-paste",
                              FALSE,
                              G_PARAM_READABLE));

    g_object_class_install_property (gobject_class, PROP_MEO_CAN_DELETE,
        g_param_spec_boolean ("meo-can-delete",
                              "meo-can-delete",
                              "meo-can-delete",
                              FALSE,
                              G_PARAM_READABLE));

    g_object_class_install_property (gobject_class, PROP_MEO_CAN_SELECT_ALL,
        g_param_spec_boolean ("meo-can-select-all",
                              "meo-can-select-all",
                              "meo-can-select-all",
                              FALSE,
                              G_PARAM_READABLE));
    ...
    signals[CLOSE] =
            g_signal_new ("close",
                      ...
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (MdAppWindowClass, close),
...
static GObject *
md_app_window_constructor (GType                  type,
                        guint                  n_props,
                        GObjectConstructParam *props)
{
    GtkWidget *vbox;
    MdAppWindow *window;
    MdAppWindowClass *klass;
    GtkAction *action;

    GObject *object =
        G_OBJECT_CLASS(md_app_window_parent_class)->constructor (type, n_props, props);

    window = MD_APP_WINDOW (object);

    klass = g_type_class_ref (type);
    md_app_window_set_id (window, md_app_window_class_get_id (klass));
    window->priv->name = g_strdup (md_app_window_class_get_name (klass));

    init_prefs (window);

    md_app_window_create_class_actions (window);
    ...
    g_signal_connect (window, "notify::toolbar-ui-name",
                      G_CALLBACK (md_app_window_update_toolbar), NULL);
    g_signal_connect (window, "notify::menubar-ui-name",
                      G_CALLBACK (md_app_window_update_menubar), NULL);
    g_signal_connect (window, "notify::ui-xml",
                      G_CALLBACK (md_app_window_update_ui), NULL);
    ...
    g_signal_connect (window, "configure-event",
                      G_CALLBACK (md_app_window_save_size), NULL);

    md_app_window_set_toolbar_visible (window,
        moo_prefs_get_bool (setting (window, PREFS_SHOW_TOOLBAR)));
    action = md_app_window_get_action (window, "ShowToolbar");
    _moo_sync_toggle_action (action, window, "toolbar-visible", FALSE);

    md_app_window_set_menubar_visible (window,
        moo_prefs_get_bool (setting (window, PREFS_SHOW_MENUBAR)));
    action = md_app_window_get_action (window, "ShowMenubar");
    _moo_sync_toggle_action (action, window, "menubar-visible", FALSE);

    md_app_window_update_ui (window);
...
static void
md_app_window_init (MdAppWindow *window)
{
    window->priv = g_new0 (MdAppWindowPrivate, 1);
    ...
    gtk_window_add_accel_group (GTK_WINDOW (window),
                                window->priv->accel_group);
}


static void
md_app_window_dispose (GObject *object)
{
    MdAppWindow *window = MD_APP_WINDOW(object);
    ...
    if (window->priv)
    {
        ...
        md_app_window_disconnect_eo_widget (window);
    ...
    G_OBJECT_CLASS (md_app_window_parent_class)->dispose (object);
}


static gboolean
md_app_window_delete_event (GtkWidget      *widget,
...
static gboolean
md_app_window_key_press_event (GtkWidget   *widget,
                            GdkEventKey *event)
{
    MdAppWindow *window = MD_APP_WINDOW (widget);

    if (window->priv->global_accels)
        return GTK_WIDGET_CLASS (md_app_window_parent_class)->key_press_event (widget, event);
    else
        return gtk_window_propagate_key_event (GTK_WINDOW (widget), event) ||
               ...
               GTK_WIDGET_CLASS (md_app_window_grand_parent_class)->key_press_event (widget, event);
}


void
md_app_window_set_global_accels (MdAppWindow *window,
                              gboolean   global)
{
    g_return_if_fail (MD_IS_APP_WINDOW (window));
    window->priv->global_accels = global != 0;
}


static gboolean
save_size (MdAppWindow *window)
{
    window->priv->save_size_id = 0;

    if (MD_IS_APP_WINDOW (window) && GTK_WIDGET_REALIZED (window))
    {
...
static gboolean
md_app_window_save_size (MdAppWindow *window)
{
    if (!window->priv->save_size_id)
...
gboolean
md_app_window_close (MdAppWindow *window)
{
    gboolean result = FALSE;
...
static void
md_app_window_set_property (GObject      *object,
                         guint         prop_id,
                         const GValue *value,
                         GParamSpec   *pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_UI_XML:
            md_app_window_set_ui_xml (window, g_value_get_object (value));
            break;

        case PROP_TOOLBAR_VISIBLE:
            md_app_window_set_toolbar_visible (window, g_value_get_boolean (value));
            break;

        case PROP_MENUBAR_VISIBLE:
            md_app_window_set_menubar_visible (window, g_value_get_boolean (value));
            break;
...
static void
md_app_window_get_property (GObject      *object,
                         guint         prop_id,
                         GValue       *value,
                         GParamSpec   *pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_ACCEL_GROUP:
            g_value_set_object (value, window->priv->accel_group);
            break;

        case PROP_TOOLBAR_UI_NAME:
            g_value_set_string (value, window->priv->toolbar_ui_name);
            break;

        case PROP_MENUBAR_UI_NAME:
            g_value_set_string (value, window->priv->menubar_ui_name);
            break;
        ...
        case PROP_ACTIONS:
            g_value_set_object (value, md_app_window_get_actions (window));
            break;
...
static void
md_app_window_update_toolbar (MdAppWindow *window)
{
    MooUIXML *xml;
    ...
    actions = md_app_window_get_actions (window);
    xml = md_app_window_get_ui_xml (window);
    ...
    gtk_toolbar_set_style (GTK_TOOLBAR (MD_APP_WINDOW(window)->toolbar), style);
}


static void
md_app_window_update_menubar (MdAppWindow *window)
{
    MooUIXML *xml;
    char *ui_name;
    MooActionCollection *actions;
...
static void
md_app_window_update_ui (MdAppWindow *window)
{
    md_app_window_update_toolbar (window);
    md_app_window_update_menubar (window);
}


static void
md_app_window_shortcuts_prefs_dialog (MdAppWindow *window)
{
    _moo_accel_prefs_dialog_run (md_app_window_get_actions (window),
                                 GTK_WIDGET (window));
}


static void
md_app_window_set_toolbar_visible (MdAppWindow  *window,
                                gboolean    visible)
{
...
static void
md_app_window_set_menubar_visible (MdAppWindow  *window,
                                gboolean    visible)
{
...
static void
toolbar_style_toggled (MdAppWindow            *window,
                       gpointer              data)
{
...
static GtkAction*
create_toolbar_style_action (MdAppWindow      *window,
...
static GtkToolbarStyle get_toolbar_style_gtk (MdAppWindow *window)
{
...
get_toolbar_style (MdAppWindow *window)
{
...
#define MD_APP_WINDOW_NAME_QUARK        (get_quark__(0))
#define MD_APP_WINDOW_ID_QUARK          (get_quark__(1))
#define MD_APP_WINDOW_ACTIONS_QUARK_    (get_quark__(2))
...
{
    return g_type_get_qdata (type, MD_APP_WINDOW_ACTIONS_QUARK_);
}
...
{
    ActionStore *store = g_type_get_qdata (type, MD_APP_WINDOW_ACTIONS_QUARK_);

    if (!store)
    {
        store = action_store_new ();
        g_type_set_qdata (type, MD_APP_WINDOW_ACTIONS_QUARK_, store);
    }
...
static const char *
md_app_window_class_get_id (MdAppWindowClass *klass)
{
    GType type;

    g_return_val_if_fail (MD_IS_APP_WINDOW_CLASS (klass), NULL);

    type = G_OBJECT_CLASS_TYPE (klass);
    return g_type_get_qdata (type, MD_APP_WINDOW_ID_QUARK);
}


static const char *
md_app_window_class_get_name (MdAppWindowClass     *klass)
{
    GType type;
    ...
    type = G_OBJECT_CLASS_TYPE (klass);
    return g_type_get_qdata (type, MD_APP_WINDOW_NAME_QUARK);
}


static void
md_app_window_class_install_action (MdAppWindowClass     *klass,
                                 const char         *action_id,
                                 ...
                                 ClosureInfo        *closure_info)
{
    ...
    g_return_if_fail (MD_IS_APP_WINDOW_CLASS (klass));
    ...
    if (g_hash_table_lookup (store->actions, action_id))
        md_app_window_class_remove_action (klass, action_id);
    ...
    for (l = window_instances; l != NULL; l = l->next)
    {
        if (g_type_is_a (G_OBJECT_TYPE (l->data), type))
        {
            ...
            if (action)
            {
                md_app_window_add_action (l->data, group, action);
...
static GtkAction *
custom_action_factory_func (MdAppWindow        *window,
                            MooActionFactory *factory)
{
    MdAppWindowActionFunc func;
    gpointer func_data;

    g_return_val_if_fail (MD_IS_APP_WINDOW (window), NULL);
...
void
md_app_window_class_new_action_custom (MdAppWindowClass     *klass,
                                    const char         *action_id,
                                    const char         *group,
                                    MdAppWindowActionFunc func,
                                    gpointer            data,
                                    GDestroyNotify      notify)
{
    ...
    md_app_window_class_install_action (klass, action_id, action_factory, group, NULL, NULL);
    g_object_unref (action_factory);
}


gboolean
md_app_window_class_find_action (MdAppWindowClass *klass,
                              const char     *id)
{
    ActionStore *store;

    g_return_val_if_fail (MD_IS_APP_WINDOW_CLASS (klass), FALSE);
...
void
md_app_window_class_remove_action (MdAppWindowClass     *klass,
                                const char         *action_id)
{
    ActionStore *store;
    GType type;
    ...
    for (l = window_instances; l != NULL; l = l->next)
        if (g_type_is_a (G_OBJECT_TYPE (l->data), type))
            md_app_window_remove_action (l->data, action_id);
}


void
md_app_window_class_new_group (MdAppWindowClass *klass,
                            const char     *name,
...
gboolean
md_app_window_class_find_group (MdAppWindowClass *klass,
                             const char     *name)
{
    ActionStore *store;
...
void
md_app_window_class_remove_group (MdAppWindowClass *klass,
                               const char     *name)
{
    ActionStore *store;
    GType type;
...
MooUIXML*
md_app_window_get_ui_xml (MdAppWindow          *window)
{
    g_return_val_if_fail (MD_IS_APP_WINDOW (window), NULL);
...
void
md_app_window_set_ui_xml (MdAppWindow          *window,
                       MooUIXML           *xml)
{
    g_return_if_fail (MD_IS_APP_WINDOW (window));
...
MooActionCollection *
md_app_window_get_actions (MdAppWindow *window)
{
    g_return_val_if_fail (MD_IS_APP_WINDOW (window), NULL);
...
GtkAction *
md_app_window_get_action (MdAppWindow  *window,
                       const char *action)
{
    g_return_val_if_fail (MD_IS_APP_WINDOW (window), NULL);
    g_return_val_if_fail (action != NULL, NULL);
...
static void
md_app_window_set_id (MdAppWindow      *window,
                   const char     *id)
{
    if (id)
    {
            ...
            MdAppWindow  *window)
{
    ...
    if (action)
    {
        md_app_window_add_action (window, info->group, action);
        g_object_unref (action);
    }
}
...
static void
md_app_window_create_class_actions (MdAppWindow *window)
{
    GType type;
...
void
md_app_window_class_set_id (MdAppWindowClass     *klass,
                         const char         *id,
                         const char         *name)
{
    GType type;
    ...
    type = G_OBJECT_CLASS_TYPE (klass);
    g_return_if_fail (g_type_get_qdata (type, MD_APP_WINDOW_ID_QUARK) == NULL);
    g_type_set_qdata (type, MD_APP_WINDOW_ID_QUARK, g_strdup (id));
    g_return_if_fail (g_type_get_qdata (type, MD_APP_WINDOW_NAME_QUARK) == NULL);
    g_type_set_qdata (type, MD_APP_WINDOW_NAME_QUARK, g_strdup (name));
}


static void
md_app_window_add_action (MdAppWindow  *window,
                       ...
                       GtkAction  *action)
{
    ...
    coll = md_app_window_get_actions (window);
    group = moo_action_collection_get_group (coll, group_name);
    g_return_if_fail (group != NULL);

    gtk_action_group_add_action (group, action);
    gtk_action_set_accel_group (action, window->priv->accel_group);
...
static void
md_app_window_remove_action (MdAppWindow  *window,
                          const char *action_id)
{
    MooActionCollection *coll;
    GtkAction *action;
...
void
md_app_window_class_new_action (MdAppWindowClass     *klass,
                              const char         *action_id,
                              const char         *group,
                              ...)
{
    ...
    md_app_window_class_install_action (klass, action_id, action_factory, group, conditions, NULL);
...
void
_md_app_window_class_new_action_callback (MdAppWindowClass *klass,
                                       const char     *id,
                                       const char     *group,
                                       GCallback       callback,
                                       GSignalCMarshaller marshal,
                                       ...
                                       ...)
{
    ...
    md_app_window_class_install_action (klass, id, action_factory,
...
static void
md_app_window_action_cut (MdAppWindow *window)
{
...
static void
md_app_window_action_copy (MdAppWindow *window)
{
    g_return_if_fail (window->priv->eo_widget != NULL);
    _moo_edit_ops_do_op (G_OBJECT (window->priv->eo_widget),
                         MOO_EDIT_OP_COPY);
...
static void
md_app_window_action_paste (MdAppWindow *window)
{
    g_return_if_fail (window->priv->eo_widget != NULL);
    _moo_edit_ops_do_op (G_OBJECT (window->priv->eo_widget),
                         MOO_EDIT_OP_PASTE);
...
static void
md_app_window_action_delete (MdAppWindow *window)
{
    g_return_if_fail (window->priv->eo_widget != NULL);
    _moo_edit_ops_do_op (G_OBJECT (window->priv->eo_widget),
                         MOO_EDIT_OP_DELETE);
...
static void
md_app_window_action_select_all (MdAppWindow *window)
{
    g_return_if_fail (window->priv->eo_widget != NULL);
    _moo_edit_ops_do_op (G_OBJECT (window->priv->eo_widget),
...
static void
emit_can_do_op_changed (MdAppWindow     *window,
                        MooEditOpType  type)
{
    switch (type)
    {
        case MOO_EDIT_OP_CUT:
            g_object_notify (G_OBJECT (window), "meo-can-cut");
            break;
        case MOO_EDIT_OP_COPY:
            g_object_notify (G_OBJECT (window), "meo-can-copy");
            break;
        case MOO_EDIT_OP_PASTE:
            g_object_notify (G_OBJECT (window), "meo-can-paste");
            break;
        case MOO_EDIT_OP_DELETE:
            g_object_notify (G_OBJECT (window), "meo-can-delete");
            break;
        case MOO_EDIT_OP_SELECT_ALL:
            g_object_notify (G_OBJECT (window), "meo-can-select-all");
            break;
        default:
            g_return_if_reached ();
    }
...
static void
md_app_window_connect_eo_widget (MdAppWindow *window,
                              GtkWidget *widget)
{
...
static void
md_app_window_disconnect_eo_widget (MdAppWindow *window)
{
    GtkWidget *widget;
    ...
    if (widget)
    {
        ...
        g_signal_handlers_disconnect_by_func (widget, (gpointer) emit_can_do_op_changed, window);
        g_object_unref (widget);
    }
}

static GtkWidget *
find_widget_for_edit_ops (MdAppWindow *window)
{
...
static void
check_edit_ops_widget (MdAppWindow *window)
{
    GtkWidget *widget;
    ...
    if (window->priv && widget != window->priv->eo_widget)
    {
        ...
        if (widget)
            md_app_window_connect_eo_widget (window, widget);
...
static void
md_app_window_set_focus (GtkWindow *window,
                      GtkWidget *widget)
{
    GTK_WINDOW_CLASS (md_app_window_parent_class)->set_focus (window, widget);
    check_edit_ops_widget (MD_APP_WINDOW (window));
}

void
md_app_window_set_edit_ops_widget (MdAppWindow *window,
                                GtkWidget *widget)
{
    g_return_if_fail (MD_IS_APP_WINDOW (window));
    g_return_if_fail (!widget || gtk_widget_get_toplevel (widget) == GTK_WIDGET (window));
    /* XXX connect to widget's destroy */
    window->priv->default_eo_widget = widget;
    check_edit_ops_widget (window);
}
